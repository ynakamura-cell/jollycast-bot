...
ADDITIONAL IC (IRREGULAR CASE) PHRASES — from指差しシート Slide 3:

② Pet isolation request (ペット隔離依頼):
"Please put your pet in a cage or another room during service. If you cannot accommodate this, I am unable to continue the service."
(サービス中はペットをケージまたは別室に隔離してください。対応いただけないとサービスできません。)

③ Heat/heatstroke prevention (熱中症対策):
"To avoid heatstroke, please turn on the AC or fan."
(熱中症対策のため、エアコンが使えない/届かない場合は扇風機等をお借りします。)

④ Outdoor work refusal in bad weather (悪天候時の屋外作業拒否):
...
WHAT THE CAST DOES: ④ Hang the laundry / ⑥ Fold the laundry
WHAT THE CUSTOMER DOES: ①②③ Operate washing machine and select detergent / ⑤ Set bathroom dryer / ⑦ Put away (store) the folded laundry

REASON: Cast cannot read kanji on washing machine controls; does not want to risk damaging important clothing.

RULES:
- Iron pressing (アイロンがけ) is NOT available — to avoid damaging important clothing
- When hanging on hangers: insert from the BOTTOM of the garment (to prevent collar stretching)
- After hanging: lightly pat/shake the garment to remove creases
- Do NOT put laundry away (storage) — ask the customer, because customers often ask where items were stored
- If the customer has a preferred folding method: ask them to demonstrate with one item as a sample

LCS PHRASES (use these when explaining to the customer):
① "I can't read kanji, so I'm not confident in operating the washing machine. Please operate it yourself."
② "To handle your important clothing properly, please also select the detergent."
③ "I will hang the clothes, but please set the bathroom dryer yourself."
④ "I cannot iron — to avoid damaging your important clothing."
⑤ "Please put away the folded clothes yourself."
...
Source: Training Booklet Day15 (Slide 41) & Day16 (Slide 43)

DAY BEFORE — 5 STEPS:
1. Check your schedule and route (confirm the service address and travel path)
2. Do a health check (confirm no fever)
3. Pre-fill customer information in your notebook
4. Send a greeting chat message to the customer via in-app chat

ON THE DAY — 5 STEPS:
1. Check in via the CaSy app (confirm you are at the correct location first)
2. Arrive 5 minutes early
3. Perform the cleaning service
4. Check out via the CaSy app
5. Write your daily report (within 24 hours)

AT THE CUSTOMER'S HOME — 6 STEPS:
1. Arrive 5 minutes early and check in
2. Ring the intercom and greet: "カジーからきました。〇〇です。〇〇様のおたくでしょうか？"
   ("I'm from CaSy. My name is 〇〇. Is this the home of 〇〇-sama?")
3. Greet at the door: "はじめまして。〇〇です。今日はよろしくおねがいします。"
   ("Nice to meet you. I'm 〇〇. Thank you for having me today.")
4. Close the door with both hands: "しつれいします。" ("Excuse me.")
5. Confirm where to put your belongings and where to wash your hands
6. When ready, let the customer know you are prepared

PRE-SERVICE MEETING — 5 STEPS:
1. "じゅんびができました。うちあわせをおねがいします。"
   ("I've finished preparing. Let's start the meeting.")
2. Confirm the cleaning area: "今日は[AREA]をそうじします。よろしいでしょうか？"
   ("Today I will clean [AREA]. Is that alright?")
3. Confirm the order: "どのじゅんばんでおそうじしましょうか？"
   ("In what order shall I clean?")
4. Walk through each room together:
   - "きになるかしょはどこですか？" ("Where are the areas you are most concerned about?")
   - "そうじどうぐはどこですか？" ("Where are the cleaning tools?")
5. Confirm at the end: where to put used cloths, and where to dispose of rubbish

=== SERVICE COMPLETION PROCEDURE ===
Source: Training Booklet Day20 (Slides 53-54)

END-OF-SERVICE WALKTHROUGH:
1. "すみません。サービスがおわりました。いっしょにごかくにんおねがいします。"
   ("Excuse me. The service is finished. Could we check together?")
2. Walk through the cleaned areas with the customer
3. Point out areas the customer was most concerned about: "ここはきになっていたかしょです。いかがでしょうか？"
   ("This was the area you were concerned about. How does it look?")
4. If any area could not be completed: report honestly and apologize
   - Include in the daily report: date/time, customer name, what was promised, what could not be done, the reason, and the customer's reaction

EXIT — 5 STEPS:
1. "ありがとうございました。" ("Thank you very much.")
2. "かえりのじゅんびします。" ("I will start packing up.")
3. Check you have not forgotten anything; gather your belongings
4. Check out via the CaSy app
5. "今日ありがとうございました。しつれいします。" ("Thank you for today. Goodbye.") → Close the door gently with both hands

DAILY REPORT: Submit within 24 hours.
...
PRE-SERVICE REMINDER (day before):
- Send a chat message to the customer the day before service:
  "Thank you for using CaSy. For tomorrow's [#]-hour service, please prepare [#] vouchers. I will collect them after the service. Thank you."
- Rule: 1 voucher = 1 hour of service
- What cast fills in on each voucher: service date/time, Service ID, Cast ID, and circle the service type (cooking/cleaning)
- After filling: place in the CaSy envelope, write cast name (in English) on the back, seal with tape, drop in a mailbox (no stamp needed)
